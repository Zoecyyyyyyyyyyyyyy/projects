...
public class PathFinder {
    ...
    public void deleteDeadEndHallway() { // delete dead end hallways
        for (int i = 0; i < allHallways.size(); i++) {
            ...
            for (int h = 0; h < world.getWidth(); h++) {
                for (int j = 0; j < world.getHeight(); j++) {
                    if (graph[h][j] == 1) {
                        if (discreteHallways(new Hallway(h, j))) {
    ...
    public boolean adjConnect(Hallway hallway) { // whether adj hallways are connected
        int col = hallway.col;
        int row = hallway.row;
    ...
    public Boolean hallwayConnectToRoom(Hallway iniHallWay, Boolean[][] whetherWalkBefore) {

        Stack<Hallway> stack = new Stack<>();
        stack.push(iniHallWay);

        while (!stack.isEmpty()) {
            Hallway current = stack.pop();
            int col = current.col;
            int row = current.row;
            ...
            if (graph[col][row] == 1) {
                stack.push(new Hallway(col + 1, row));
                stack.push(new Hallway(col - 1, row));
                stack.push(new Hallway(col, row + 1));
                stack.push(new Hallway(col, row - 1));
            }
        }
        return false;
